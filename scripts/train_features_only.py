...
from tracer.models.dual_stream import FeaturesOnlyPredictor
...
def main():
    ...
    logger = setup_logging(log_dir, "train_features_only")
    ...
    model = FeaturesOnlyPredictor(dict(cfg.model.stream_a))
    model.to(device)
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for batch in train_loader:
            ...
            optimizer.zero_grad()
            logits = model(features, traj_mask).unsqueeze(1)
        ...
        if auc > best_auc:
            best_auc = auc
            ckpt_path = os.path.join(out_dir, "features_only.pt")
            save_checkpoint(
                model,
                optimizer,
                scheduler,
            ...
            logger.info("Saved best features-only checkpoint to %s", ckpt_path)

        if early_stopping(auc):
            logger.info("Early stopping at epoch %d", epoch)
            break

    logger.info("Features-only training finished. Best dev AUC: %.4f", best_auc)
