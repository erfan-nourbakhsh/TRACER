...
from tracer.models.dual_stream import TextOnlyPredictor
...
def main():
    ...
    logger = setup_logging(log_dir, "train_text_only")
    ...
    if args.dataset_filter != "all":
        ...
        train_features = {
            d.dialogue_id: train_features[d.dialogue_id]
            for d in train_dialogues
            if d.dialogue_id in train_features
        }
        dev_features = {
            d.dialogue_id: dev_features[d.dialogue_id]
            for d in dev_dialogues
            if d.dialogue_id in dev_features
        }
    ...
    tokenizer = BeliefStateEncoder.get_tokenizer(
        cfg.model.stream_b.get("encoder_name", "roberta-base")
    )
    ...
    model = TextOnlyPredictor(dict(cfg.model.stream_b))
    model.to(device)
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for batch in train_loader:
            ...
            optimizer.zero_grad()
            logits = model(input_ids=input_ids, attention_mask=attention_mask).unsqueeze(
                1
            )
        ...
        eds_result = compute_early_detection_score(
            dial_preds, dial_labels, threshold=0.5
        )
        logger.info(
        ...
        if auc > best_auc:
            best_auc = auc
            ckpt_path = os.path.join(out_dir, "text_only.pt")
            save_checkpoint(
                model,
                optimizer,
                scheduler,
                epoch,
                {"auc": auc, "eds": eds_result["eds"]},
                ckpt_path,
            )
            logger.info("Saved best text-only checkpoint to %s", ckpt_path)

        if early_stopping(auc):
            logger.info("Early stopping at epoch %d", epoch)
            break

    logger.info("Text-only training finished. Best dev AUC: %.4f", best_auc)
